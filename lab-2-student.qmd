...
```
Reflection: In question 1, I did not correctly read in the data. I did not use the here() function, but rather used a relative path. I did not fully understand how to read in data, but now realize it is beneficial to use the here() function because it findas the root directory of the project, making it easier to locate the data files. 
...
This dataset has 30463 rows and 15 columns. 

Reflection: Originally, I did not include any code to provide the reader with information about the data set. I added the glimpse() function to show a concise representation of the data, which includes the dimensions of the data. I now realize that while I could see the details of the data set because I had direct access to the data file, the readers of my Quarto document were not able to see it. By including the glimpse() function, I am able to provide insight about the data set being used to the reader such as the dimensions of the data.

**3. What are the data types of the variables in this dataset?**

<!-- You can either write code here or in the code chunk for #1, but I **must** see the code you used to address this question!  -->
The data types are chr, which is a categorical variable, dbl, which is a continuous numerical variable, and date. 

Reflection: Similarly to question 2, I did not include any code to provide the reader with information about the data set. I added the glimpse() function to show a concise representation of the data, which includes the data types of the variables in the data set. Again, I understand now that while I could see the details of the data set because I had direct access to the data file, the readers of my Quarto document were not able to see it. By including the glimpse() function, I am able to provide insight about the data set being used to the reader such as the data types of the variables. 
...
Reflection: I used the wrong variable originally. This led me to produce an incorrect visual. In my revision, I made sure to use the correct variable and will pay closer attention to the instructions to ensure I am using the correct variable. 
...
Reflection: I accidentally used the species_id variables rather than the species variable in my scatterplot. In my revision, I fixed this by substituting species_id for species. For the future, I will make sure to read the instructions more carefully so that I am using the correct variable.

**7. No plot is complete without axis labels and a title. Include reader friendly labels and a title to your plot.**

Reflection: I originally did not include specific enough labels in my plots and did not incorporate the term, rodent, which is the type of species being examined.. I added the word rodent instead species in my plot title to specify the taxon of of species and changed "Weight of Animal (in grams)" to  "Weight of Rodent (in grams)" to make it more clear and specific. I understand now that it is important to be as specific and clear in my visuals to make it as simple and easy for the reader to interpret. 
...
Reflection: As I explained earlier, I accidentally used the species_id variables rather than the species variable in my scatterplot. In my revision, I fixed this by substituting species_id for species. For the future, I will make sure to read the instructions more carefully so that I am using the correct variable.
...
Reflection: I changed my alpha from 0.5 to 0.1 to make the points more transparent. I now see that making the points more transparent makes it easier to see the relations between the information the boxplot presents, including the five number summary, and the observations. 
...
Reflection: When I used species_id, the categories of the species_id variable did not overlap, but once I used the correct species variable, the categories overlapped. Considering this, I used   theme(axis.text.x = element_text(angle = 45) to turn the species axis labels 45 degrees. By incorporating this in my revision, all of the labels are readable for the viewer, making the visual more easy to understand. Additionally, with my plot titles and axis labels, I simplified the weight axis label to just say "Weight (in grams)" rather than "Weight of Rodent (in grams)" to be less repetitive. I did not realize that these labels were repetitive, but now see this and realize I should have my labels be as simple as possible, while still providing sufficient information to the reader.
...
Based on a p-value of 0.0000000000000002, which is a smaller value than the significance level of 0.001, we have sufficient evidence to reject the null hypothesis. This means that we have sufficient evidence that there is a significant difference in means of at least one rodent species compared to the others. 

Reflection: I forgot to include "difference in means" in my response. The anova test is used to see if there is a difference in means between different groups and I simply said there is a difference between groups. In my revision, I made sure to include "difference in means" because this is a key aspect of what the anova test shows and it is inaccurate to just say there is a "difference between at least one rodent species compared to the others" based on the anova test without including the aspect about the "difference in means." 
